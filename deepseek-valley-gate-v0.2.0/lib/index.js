...
// 全屏遮罩脚本（mode=manual 用窗口分钟判定；mode=official 用 UTC 高峰窗口判定）。
function overlayJs(mode, startMinute, endMinute, peakWindows, weekdaysOnly) {
	const windowsJson = JSON.stringify(peakWindows || [[1, 4], [6, 10]]);
	return `(function () {
	if (window.__dshValleyGate) return
	window.__dshValleyGate = true
	var MODE = ${JSON.stringify(mode || "manual")}
	var START = ${startMinute}
	var END = ${endMinute}
	var PEAK = ${windowsJson}
	var WEEKDAYS_ONLY = ${weekdaysOnly !== false}
	function minutes() {
		var d = new Date()
		return ((d.getUTCHours() + 8) * 60 + d.getUTCMinutes()) % 1440
	}
	function inPeak() {
		var d = new Date()
		var dow = d.getUTCDay()
		if (WEEKDAYS_ONLY && !(dow >= 1 && dow <= 5)) return false
		var h = d.getUTCHours() + d.getUTCMinutes() / 60
		for (var i = 0; i < PEAK.length; i++) {
			var s = PEAK[i][0], e = PEAK[i][1]
	...
	function inManualWindow() {
		...
		var span = END - START
		if (span >= 1440) return true
		var offset = ((m - START) % 1440 + 1440) % 1440
		return offset < span
	}
	function active() {
		if (MODE === 'official') return !inPeak()
		return inManualWindow()
	}
	function pad(n) { return (n < 10 ? '0' : '') + n }
	function until() {
		var m = minutes()
		var target = m < START ? START : START + 1440
		var mins = target - m
		var d = new Date()
	...
	function build() {
		...
		p.textContent = '本 DSH 已开启低谷时段门禁：仅低谷时段允许运行，以节省费用。窗口与开关可在「设置 → 低谷门禁」里调整。'
		var c = document.createElement('div')
		c.setAttribute('data-countdown', '')
		c.style.cssText = 'font-size:32px;font-weight:700;font-variant-numeric:tabular-nums;color:#58a6ff'
		c.textContent = '--:--:--'
...
function apply(ctx) {
	...
	ctx.inject(["webServer"], (webCtx) => {
		...
		disposers.push(webServer.tapIndex((html) => {
			...
			const tag = `<script defer src="/deepseek-valley-gate/overlay.js?mode=${mode}&start=${config.startMinute}&end=${config.endMinute}"></script>`;
		...
		disposers.push(webServer.register({
			...
			handler: (req, res) => {
				try {
					...
					const start = Number(url.searchParams.get("start")) || config.startMinute;
					const end = Number(url.searchParams.get("end")) || config.endMinute;
					// official 模式下 overlay 也按官方规则判定（UTC 高峰窗口）。
					res.writeHead(200, {
						"Content-Type": "application/javascript; charset=utf-8",
						"Cache-Control": "no-store"
					});
					res.end(overlayJs(mode, start, end, official.peakWindows, official.weekdaysOnly));
				} catch (_e) {
					res.writeHead(500);
